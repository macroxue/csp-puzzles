...
SendMoreMoney::SendMoreMoney(Option option) : Problem<int>(option) {
  ...
  for (int i = 0; i < 8; i++) different->AddVariable(v[i]);
  ...
  for (int i = 0; i < 8; i++) sum->AddVariable(v[i]);
  AddConstraint(sum);
}
...
bool SendMoreMoney::Sum::operator()(int count, const int values[], int) {
  return values[0] * 1000 + values[1] * 100 + values[2] * 10 + values[3] +
         ...
         values[4] * 10000 + values[5] * 1000 + values[2] * 100 +
             values[1] * 10 + values[7];
}

void SendMoreMoney::ShowSolution() {
  printf("     S=%d E=%d N=%d D=%d\n", s->GetValue(0), e->GetValue(0),
         n->GetValue(0), d->GetValue(0));
  printf(" +)  M=%d O=%d R=%d E=%d\n", m->GetValue(0), o->GetValue(0),
         r->GetValue(0), e->GetValue(0));
  printf("------------------------\n");
  printf(" M=%d O=%d N=%d E=%d Y=%d\n", m->GetValue(0), o->GetValue(0),
         n->GetValue(0), e->GetValue(0), y->GetValue(0));
}
