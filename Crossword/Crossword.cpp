...
Crossword::Crossword(Option option) : Problem<char>(option) {
  // Letters as variables
  Variable<char> *l1, *l2, *l3, *l4, *l5, *l6, *l7, *l8, *l9, *l10, *l11, *l12,
      *l13, *l14, *l15, *l16;
  memset(v, 0, sizeof(v));
...
bool Crossword::Word::operator()(int count, const char values[], char) {
  ...
  for (int i = 0; i < 5 && words[count - 3][i]; i++)
    if (strncmp(values, words[count - 3][i], count) == 0) return true;
  return false;
}
