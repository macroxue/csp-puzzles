...
struct Code {
  ...
  void Randomize() {
    ...
    for (int i = 0; i < num_pegs; i++) pegs[i] = colors[i];
  }
...
MasterMind::MasterMind(Option option, int num_pegs, int num_colors)
  ...
  for (int i = 0; i < num_pegs; i++) c->AddVariable(v[i]);
  AddConstraint(c);
}
...
void MasterMind::ShowSolution() {
  ...
  if (auto_play) {
    ...
    int num_items;
    do
      num_items = scanf("%1d%1d", &guess.num_correct, &guess.num_incorrect);
    while (num_items != 2);
  }
  if (guess.num_correct == num_pegs) {
    ...
    for (int i = 0; i < num_pegs; i++) match->AddVariable(v[i]);
    AddConstraint(match);
  }
}

bool MasterMind::Match::operator()(int count, const int values[],
                                   int guess_id) {
  Code &guess = guesses[guess_id];
  int num_correct, num_incorrect;
  guess.Match(values, num_correct, num_incorrect);
  return num_correct == guess.num_correct &&
         num_incorrect == guess.num_incorrect;
}
...
int main(int argc, char *argv[]) {
  ...
  for (int game = 1; game <= 1000; game++) {
    ...
    if (guesses[num_guesses - 1].num_correct < num_pegs) {
      ...
      if (max_guesses < num_guesses) max_guesses = num_guesses;
